...
router = APIRouter(prefix="/license-plate", tags=["License-plate"])
...
def license_plate_list(
    license_plate: schemas.LicensePlateCreate,
    db: Session = Depends(get_db),
    current_user: int = Depends(oauth2.get_current_user),
):
    new_license_plate = models.LicensePlate(
        created_by=current_user.id, **license_plate.dict()
    )
...
def update_license_plate(
    id: int,
    updated_license_plate: schemas.LicensePlateCreate,
    db: Session = Depends(get_db),
    current_user: int = Depends(oauth2.get_current_user),
):
...
def license_plate_controller_in(
    license_plate_number: schemas.LicensePlateNumber, db: Session = Depends(get_db)
):
    print(license_plate_number)
    print(license_plate_number.license_plate)
    ps = (
        db.query(models.LicensePlate)
        .filter(models.LicensePlate.license_plate == license_plate_number.license_plate)
        .first()
    )
    if ps is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"License plate with id {license_plate_number} not found",
        )
    ps.active_now = True
